...
def create_report(inference, header, orig_vol, pred_vol):
    ...
    # identify 3 spots to draw in 1000,1000. Lets use image size of 300,300
    # [30,280],[350,280],[670,280] 
    ...
    for idx in range(len(slice_nums)):
        ...
        #np_pred = np.array(pil_pred.getdata())
        #print(np.unique(nd_pred))
        #print(np_pred.shape)
...
if __name__ == "__main__":
    ...
    # running inference. Your job is to implement it. - Completed
    pred_label = inference_agent.single_volume_inference_unpadded(np.array(volume))
    # TASK: get_predicted_volumes is not complete. Go and complete it - Completed
